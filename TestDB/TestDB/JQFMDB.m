...
- (BOOL)jq_createTable:(NSString *)tableName dicOrModel:(id)parameters excludeName:(NSArray *)nameArr
{
    ...
    BOOL creatFlag;
//    fieldStr = [fieldStr stringByReplacingOccurrencesOfString:@"message_id TEXT" withString:@"message_id TEXT UNIQUE"];
...
{
    NSString *sqlstr = [NSString stringWithFormat:@"drop database %@", dbName];
    if (![_db executeUpdate:sqlstr])
    {
        return NO;
    }
