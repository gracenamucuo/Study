...
//            self.userInteractionEnabled = NO;
            ...
            StyleModel *model = (StyleModel *)[cell valueForKeyPath:@"model"];
            if (model.type == 2 || model.content.length <= 0) {
                return;
            }
        ...
        default:
        {
            ...
            [self.snapedView removeFromSuperview];
             self.currentIndexPath = [self _getChangedIndexPath];
            if ([self.operateDelegate respondsToSelector:@selector(moveEndOldIndexPath:currentIndexPath:)]) {
                
                [self.operateDelegate moveEndOldIndexPath:self.oldIndexPath currentIndexPath:self.currentIndexPath];
                [self reloadData];
            }
        }
            break;
